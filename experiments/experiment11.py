...
wandb.init(project="cifar100-noisy", name="resnet50_fd_e12")
...
def filter_noisy_data(dataset, threshold=0.6):
    filtered_data = []
...
filtered_train_data = filter_noisy_data(train_set)
train_set.data, train_set.targets = zip(*filtered_train_data)
...
df.to_csv("kaggle\\working\\submission12.csv", index=False)

wandb.save("kaggle\\working\\submission12.csv")
# %%