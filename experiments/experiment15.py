...
wandb.init(project="cifar100-noisy", name="experiment17_cd")
...
train_dataset = SimpleCachedDataset(train_dataset, train_transform, False)
test_dataset = SimpleCachedDataset(val_dataset, None, False)
...
mentor_optimizer = optim.Adam(mentor_net.parameters(), lr=1e-4)
...
def train():
    ...
    for inputs, targets in tqdm(train_loader, desc="Training", leave=False):
        ...
        mentor_inputs = model(inputs)
        mentor_scores = mentor_net(mentor_inputs).squeeze()

        weights = mentor_scores / mentor_scores.sum()
...
df.to_csv("kaggle\\working\\submission17.csv", index=False)

wandb.save("kaggle\\working\\submission17.csv")
